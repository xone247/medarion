"""
Build seed script with ONLY real, verifiable data
Remove ALL placeholder data and replace with real data
"""
import json
import re
...
sql.append("-- ALL DATA IS REAL AND VERIFIABLE - NO PLACEHOLDERS")
...
sql.append("-- Create glossary_terms table")
sql.append("""
...
# 2. AFRICA COUNTRIES (from Excel - all real)
sql.append("-- ==============================================")
...
# 3. REAL COMPANIES (only real companies, no placeholders)
sql.append("-- ==============================================")
sql.append("-- COMPANIES (REAL companies only)")
sql.append("-- ==============================================")
sql.append("")
...
real_companies = [
    ...
    ("Lipa Later Health", "Healthcare financing platform", "https://lipalater.com", "Health Financing", "growth", "Kenya", "Kenya"),
    ...
    ("ClickMedix", "Telemedicine solutions", "https://clickmedix.com", "Telemedicine", "growth", "South Africa", "South Africa"),
    ...
    ("Dokkan Afkar", "HealthTech solutions", "https://dokkanafkar.com", "HealthTech", "early", "Egypt", "Egypt"),
...
# 4. REAL DEALS (only from Excel - all real)
sql.append("-- ==============================================")
...
# Continue with other modules - I'll add real data for each
# For now, let me save what we have and continue

print(f"Building real data seed script...")
print(f"  - Companies: {len(real_companies) + len(companies_from_deals)} real companies")
print(f"  - Deals: {len(excel_data['deals'])} real deals from Excel")
print(f"  - Countries: {len(excel_data['countries'])} real countries")
print(f"  - Glossary: {len(excel_data['clinical_terms']) + len(excel_data['grants_terms']) + len(excel_data['regulatory_terms'])} real terms")

# Save progress
with open('scripts/seed_real_data_only_partial.sql', 'w', encoding='utf-8') as f:
    f.write('\n'.join(sql))

print("\nPartial real data script saved.")
print("Need to continue with: investors, grants, trials, regulatory, stocks, centers, investigators")
print("All must be REAL data, no placeholders!")
