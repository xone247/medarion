#!/bin/bash
# Setup SSH tunnel to Vast.ai with proper key handling
# This version handles SSH key setup if needed
...
if [ -f "$SSH_KEY_PATH" ]; then
    ...
    echo "⚠️  SSH key not found, will use password authentication"
    KEY_OPTION=""
fi
...
while true; do
    ...
    ssh \$KEY_OPTION -N -L \${LOCAL_PORT}:localhost:\${REMOTE_PORT} \
...
echo "🚀 Starting tunnel in screen session..."
