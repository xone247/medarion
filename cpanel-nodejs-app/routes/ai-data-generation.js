...
// AI Model Configuration - Support Vast.ai, SageMaker, and vLLM/Ollama
...
const AI_CONFIG = {
  ...
  useSageMaker: !(process.env.AI_MODE === 'vast' || !!process.env.VAST_AI_URL) && (process.env.AI_MODE === 'cloud' || !!process.env.SAGEMAKER_ENDPOINT_NAME),
...
async function callAI(prompt, systemPrompt = null) {
  ...
  // Use SageMaker if configured for cloud mode
  if (AI_CONFIG.useSageMaker) {
    try {
      ...
      console.error('SageMaker call error:', error);
      throw error;
    }
  }
...
router.get('/health', async (req, res) => {
  try {
    ...
    let aiMode = AI_CONFIG.useVastAi ? 'vast' : (AI_CONFIG.useSageMaker ? 'cloud' : 'local');
    ...
    } else if (AI_CONFIG.useSageMaker) {
      // Test SageMaker
      aiAvailable = await sagemakerService.healthCheck();
      aiEndpoint = `sagemaker:${process.env.SAGEMAKER_ENDPOINT_NAME || 'mistral-7b-serverless'}`;
    } else {
      // Test vLLM/Ollama
      try {
