...
async function callAI(prompt, systemPrompt = null, useRAG = true) {
  ...
  console.log('[AI Update] Calling AI model, useVastAi:', useVastAi);
  ...
  try {
    ...
    if (error.message.includes('ECONNREFUSED') || error.message.includes('fetch failed')) {
      ...
      return `Demo response: The AI service is not currently configured. Please set up your AI endpoint (Vast.ai or local vLLM/Ollama) to enable real data updates.`;
    }
  ...
  if (!Array.isArray(updates) || updates.length === 0) {
    ...
    if (response && (response.includes('Demo response') || response.includes('not currently configured'))) {
      return { 
        count: 0, 
        updated: [], 
        message: 'AI service is not configured. Please set up your AI endpoint (Vast.ai or local vLLM/Ollama) to enable data updates.',
        warning: true
      };
    }
...
// Update Fundraising CRM - Add new CRM investors (note: this is user-specific, so we'll create template entries)
async function updateFundraisingCRM(count = 5) {
  const prompt = `Provide ${count} REAL investor profiles that could be added to a fundraising CRM. Use actual investors, VCs, and funds that invest in African healthcare.
...
[
  {
    "name": "Investor Name",
    "type": "VC|PE|Angel|Corporate",
    "focus": "Healthcare|Telemedicine|AI Diagnostics",
    ...
    "deal_size": "$100K - $1M",
    "timeline": "3-6 months",
    "notes": "Investment focus and criteria"
  }
]`;

  const systemPrompt = 'You are an expert in African healthcare investment with access to real investor data. You MUST provide ONLY REAL, FACTUAL investors, VCs, and funds that actually exist and invest in African healthcare. Use your training data and knowledge base. Do NOT generate fictional investors.';
  const response = await callAI(prompt, systemPrompt);
  const investors = parseAIResponse(response);
  ...
  // Note: CRM investors are user-specific, so we return the data structure
  // The actual insertion should be done by the user or admin
  return { count: investors.length, investors, note: 'CRM investors are user-specific. Use this data to populate user CRM records.' };
}

// Update Glossary - Add new glossary terms
async function updateGlossary(count = 10) {
  const prompt = `Provide ${count} REAL, FACTUAL glossary terms related to healthcare funding, regulation, and clinical research in Africa. Use actual industry terms and definitions.

IMPORTANT: Use ONLY REAL terms with accurate definitions:
[
  {
    "term": "Term Name",
    "definition": "Accurate definition of the term",
    "category": "funding|regulation|clinical|business|technical",
    "examples": "Usage examples",
    "source": "Source of definition"
  }
]`;

  const systemPrompt = 'You are an expert in healthcare industry terminology with access to real definitions and standards. You MUST provide ONLY REAL, FACTUAL terms with accurate definitions from industry standards, regulations, and best practices. Use your training data and knowledge base. Do NOT generate fictional terms.';
  const response = await callAI(prompt, systemPrompt);
  const terms = parseAIResponse(response);
    ...
    try {
      const [result] = await conn.execute(
        `INSERT INTO glossary_terms (
          term, definition, category, examples, source, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, NOW(), NOW())
        ON DUPLICATE KEY UPDATE definition = VALUES(definition), updated_at = NOW()`,
        [
          term.term,
          term.definition,
          term.category || 'funding',
          term.examples || null,
          term.source || null
        ]
      );
      inserted.push({ id: result.insertId || 'updated', term: term.term });
    } catch (error) {
      console.error(`Error inserting glossary term:`, error.message);
    }
  }

  await conn.end();
  return { count: inserted.length, terms: inserted };
}
...
router.post('/:module', async (req, res) => {
  try {
    ...
    switch (module) {
      ...
      case 'fundraising-crm':
      case 'crm-investors':
        result = await updateFundraisingCRM(count);
        break;
      case 'glossary':
        result = await updateGlossary(count);
        break;
      default:
...
router.post('/all', async (req, res) => {
  try {
    ...
    results.fundraising_crm = await updateFundraisingCRM(counts.fundraising_crm || 5);
    results.glossary = await updateGlossary(counts.glossary || 10);
