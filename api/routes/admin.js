...
router.post('/blog-posts', authenticateToken, async (req, res) => {
  try {
    const { title, excerpt, content, author, category, readTime, imageUrl, featured, status } = req.body;
    ...
    const [result] = await db.execute(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [title, slug, content, excerpt || '', author || 'Admin', imageUrl || '', category || 'General', status || 'draft', status === 'published' ? new Date() : null]);
...
router.put('/blog-posts/:id', authenticateToken, async (req, res) => {
  try {
    ...
    await db.execute(`
      UPDATE blog_posts 
      SET title = ?, slug = ?, content = ?, excerpt = ?, author_id = ?, featured_image = ?, category = ?, status = ?, updated_at = NOW()
      WHERE id = ?
    `, [title, slug, content, excerpt || '', author || 'Admin', imageUrl || '', category || 'General', status || 'draft', id]);
...
router.post('/companies', authenticateToken, async (req, res) => {
  try {
    const { name, description, website, sector, stage, country, total_funding, employees_count, is_active } = req.body;
    ...
    const [result] = await db.execute(
      `INSERT INTO companies (name, description, website, sector, stage, country, total_funding, employees_count, is_active)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, description, website, sector, stage, country, total_funding, employees_count, is_active !== false]
    );
  ...
  try {
    ...
    await db.execute(
      `UPDATE companies SET name = ?, description = ?, website = ?, sector = ?, stage = ?, country = ?, 
       total_funding = ?, employees_count = ?, is_active = ?, updated_at = NOW() WHERE id = ?`,
      [name, description, website, sector, stage, country, total_funding, employees_count, is_active !== false, id]
    );
